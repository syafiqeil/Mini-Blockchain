...
async fn main() {
    let subscriber = FmtSubscriber::builder()
        ...
        .with_target(true)
        .finish();
    tracing::subscriber::set_global_default(subscriber).expect("Gagal meng-set subscriber logging");
    ...
    if args.is_authority {
        ...
        info!("Alamat Otoritas: 0x{}", hex::encode(authority_keypair.public_key));

        let chain_clone = Arc::clone(&blockchain);
        let key_clone = Arc::clone(&authority_keypair);
        let mempool_clone = Arc::clone(&mempool);
        // Clone pengirim untuk task otoritas
                ...
                if !transactions.is_empty() {
                    let new_block = {
                        ...
                        if chain.add_block(block.clone()) {
                            ...
                            error!(
                                "OTORITAS: Gagal menambahkan blok yang baru dibuat ke chain lokal."
                            );
                            continue;
                        }
                    };
                    // Kirim blok ke kanal P2P
                    if tx_p2p_clone_auth.send(ChainMessage::NewBlock(new_block)).await.is_err() {
                        // Channel ditutup, task P2P mungkin sudah berhenti
                        break;
                    }
                }
            }
    ...
    // RPC akan memegang 'tx_p2p' yang asli, menjaga agar kanal tetap hidup.
